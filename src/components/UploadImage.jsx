...
import { supabase } from "../supabase"; // Pastikan ada file supabase.js
...
function UploadImage() {
  ...
  const listImages = async () => {
    const { data, error } = await supabase.storage
      .from("foto-siswa") // Bucket Supabase
  ...
  const uploadImage = async () => {
    ...
    const { error } = await supabase.storage
      .from("foto-siswa")
      .upload(`GambarAman/${fileName}`, imageUpload);
    ...
    const { data: publicUrl } = supabase.storage
      .from("foto-siswa")
      .getPublicUrl(`GambarAman/${fileName}`);

    setImageList((prev) => [...prev, publicUrl.publicUrl]);
